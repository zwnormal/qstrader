...
def test_sim_event_eq(sim_event_params, compare_event_params, expected_result):
    """
    Checks that the SimulationEvent __eq__ correctly
    compares SimulationEvent instances.
    """
    sim_event = SimulationEvent(
        pd.Timestamp(sim_event_params[0], tz=pytz.UTC), sim_event_params[1]
    )
    compare_event = SimulationEvent(
        pd.Timestamp(compare_event_params[0], tz=pytz.UTC), compare_event_params[1]
    )
